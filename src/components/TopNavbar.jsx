...
const TopNavbar = ({ currentUser, setCurrentUser, systemUsers = [], workers = [], setAuthenticated }) => {
  ...
  const loadNotifications = () => {
    if (currentUser?.id) {
      setNotifications(getNotificationsForUser(currentUser.id, systemUsers))
    } else {
      setNotifications([])
    }
  }
  ...
  const handleLogout = () => {
    localStorage.removeItem('alfa_authenticated');
  ...
  return (
    <header className="flex justify-between items-center w-full px-6 h-16 sticky top-0 z-40 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                              Dari: <span className="font-bold text-slate-700">{notif.submittedBy}</span>
                            </p>
