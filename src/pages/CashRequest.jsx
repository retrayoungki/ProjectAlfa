...
const CashRequest = ({ projects = [], workers = [], currentUser, systemUsers = [] }) => {
  ...
  return (
    <div className="p-lg max-w-[1200px] mx-auto">
      ...
      <div className="bg-white shadow-2xl border border-slate-200 p-12 min-h-[1200px] text-slate-900 relative print:shadow-none print:border-none print:p-0">
        
        {/* Status Watermark */}
        {status !== 'Draft' && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rotate-[-12deg] pointer-events-none z-10 opacity-50 print:opacity-50 whitespace-nowrap">
        ...
        <div className="grid grid-cols-2 gap-x-12 gap-y-2 mb-8 text-sm">
          <div className="space-y-1">
            <div className="flex">
              ...
              <input className="flex-1 border-b border-transparent hover:border-slate-300 outline-none bg-transparent font-bold" value={vendor} onChange={e => status === 'Draft' && setVendor(e.target.value)} readOnly={status !== 'Draft'} />
            </div>
            <div className="flex">
              <span className="w-32 font-black uppercase">Address</span><span className="mr-2">:</span>
              <input className="flex-1 border-b border-transparent hover:border-slate-300 outline-none bg-transparent font-bold" value={address} onChange={e => status === 'Draft' && setAddress(e.target.value)} readOnly={status !== 'Draft'} />
            </div>
            <div className="flex">
              <span className="w-32 font-black uppercase">Telephone</span><span className="mr-2">:</span>
              <input className="flex-1 border-b border-transparent hover:border-slate-300 outline-none bg-transparent font-bold" value={telephone} onChange={e => status === 'Draft' && setTelephone(e.target.value)} readOnly={status !== 'Draft'} />
            </div>
          </div>
          <div className="space-y-1">
            ...
            <div className="flex">
              ...
              <select className="flex-1 border-b border-transparent outline-none bg-transparent font-bold uppercase" value={project} onChange={e => status === 'Draft' && setProject(e.target.value)} disabled={status !== 'Draft'}>
                <option value="">...</option>
            ...
            <div className="flex">
              ...
              <div className="flex-1 flex flex-col gap-1">
                <select
                  className={`w-full border-b border-transparent hover:border-slate-300 outline-none bg-transparent font-bold uppercase ${status !== 'Draft' ? 'cursor-default' : 'cursor-pointer'}`}
                  value={scopeOfWork}
            ...
            {items.map((item, index) => (
              <tr key={item.id} className="border-b-2 border-slate-900 last:border-b-0">
                ...
                <td className="border-r-2 border-slate-900 px-6 py-6 align-top">
                  <textarea className="w-full border-none outline-none bg-transparent italic font-medium" rows={2} value={item.description} onChange={e => status === 'Draft' && handleItemChange(item.id, 'description', e.target.value)} readOnly={status !== 'Draft'} placeholder="Payment for service..." />
                  {item.descriptionImage && <img src={item.descriptionImage} className="mt-4 max-w-xs rounded border" alt="attachment" />}
                  {status === 'Draft' && !item.descriptionImage && (
      ...
      {status === 'Draft' && (
        <div className="no-print mt-8 bg-white border border-slate-200 rounded-2xl p-6 flex justify-end gap-3 shadow-xl">
          ...
          <button onClick={handleSubmit} className="px-8 py-2.5 bg-primary text-white rounded-lg font-black text-xs uppercase tracking-widest shadow-lg hover:brightness-110 transition-all flex items-center gap-2">
            <span className="material-symbols-outlined text-sm">send</span> Submit for Approval
          </button>
        </div>
      )}
