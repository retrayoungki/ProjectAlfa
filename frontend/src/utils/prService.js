 ...
 */
export const applyApproval = (prRef, approver, decision, comment = '', systemUsers = [], priorityStatus = null) => {
  ...
  if (level === 1) {
    pr.approval1 = entry
    // Save priority signal if provided (usually from Senior PM)
    if (priorityStatus) {
      pr.priorityStatus = priorityStatus
    }
  }
 ...
 */
export const applyPayment = (prRef, admin, notes = '', proofImage = null) => {
...
export const getNotificationsForUser = (userId, systemUsers = []) => {
  ...
  return notifs
    ...
    .map(n => {
      ...
      if (n.type.startsWith('pr_') && (!n.submittedBy || n.submittedBy === 'Unknown')) {
...
/**
 * Shared sorting logic for PR/CR lists.
 * Priority: 
 * 1. Priority Signal ('Priority') AND NOT Paid
 * 2. Regular date-based sorting
 */
export const getSortedPRs = (prs) => {
  return [...prs].sort((a, b) => {
    const aIsPriority = a.priorityStatus === 'Priority' && a.status !== 'Paid'
    const bIsPriority = b.priorityStatus === 'Priority' && b.status !== 'Paid'
