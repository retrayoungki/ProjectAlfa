...
const CashRequest = ({ projects = [], workers = [], currentUser, systemUsers = [] }) => {
  ...
  useEffect(() => {
    if (project) {
      ...
      // Calculate scope-specific spending (global check)
      if (scopeOfWork) {
  ...
  const handleSubmit = () => {
    ...
    // New validation: Enforce Planned Scope Limit (Cost Engine Integration)
    if (plannedLimit > 0) {
      const allPRs = getAllPRs()
    ...
    setHistory(getSortedPRs(getAllPRs().filter(p => p.type === 'Cash Request')))
  }
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {cr.priorityStatus === 'Priority' && cr.status !== 'Paid' && (
