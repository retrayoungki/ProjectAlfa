...
// ─── Main Component ───────────────────────────────────────────────────────────
const CostManagement = ({ projects, setProjects, currentUser }) => {
  ...
  const syncBudget = React.useCallback((newBudget) => {
    if (setProjects && selectedProject) {
      setProjects(prev => prev.map(p => 
        String(p.id) === String(selectedProject) ? { ...p, budget: String(Math.round(newBudget)) } : p
      ))
    }
  ...
  const {
    ...
    handleImportExcel, handleSaveRevision, handleRestoreRevision,
  } = useCostEstimation(selectedProject, currentUser, syncBudget)
      ...
      <div className="p-6 border-b border-slate-200 bg-white shrink-0 z-10 shadow-sm">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-5">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            >
              <span className="material-symbols-outlined text-base">upload_file</span>Import Excel
            </button>
            <input
              ref={fileInputRef}
              type="file"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-50 border border-slate-100 p-3 rounded">
            <p className="text-[10px] font-bold text-slate-700 uppercase tracking-widest mb-1">Total Cost</p>
            <p className="font-black text-slate-800 text-sm tabular-nums">{fmt(baseCost)}</p>
          </div>
          {[
            { label: 'Profit Margin', field: 'profit', cost: profitCost },
            ...
            <div key={field} className="bg-slate-50 border border-slate-100 p-3 rounded">
              ...
              <p className="font-bold text-red-600 text-sm tabular-nums">- {fmt(cost)}</p>
            </div>
          ))}
          <div className="bg-blue-50 border border-blue-100 p-3 rounded">
            <p className="text-[10px] font-black text-blue-800 uppercase tracking-widest mb-1">Nett Total</p>
            <p className="font-black text-blue-900 text-base tabular-nums">{fmt(baseCost - profitCost - taxCost)}</p>
          </div>
        </div>
      </div>
              ...
              <div className="overflow-auto flex-1">
                <table className="w-full text-left border-collapse min-w-[600px]">
                  <thead className="bg-slate-100 sticky top-0 z-10 shadow-sm">
                    <tr>
                      <th className="px-4 py-3 text-[10px] font-black text-slate-500 uppercase tracking-wider w-16 border-r border-slate-200 text-center">No</th>
                      <th className="px-4 py-3 text-[10px] font-black text-slate-500 uppercase tracking-wider border-r border-slate-200">Item Description</th>
                      <th className="px-4 py-3 text-[10px] font-black text-slate-500 uppercase tracking-wider w-32 border-r border-slate-200 text-center">Quantity</th>
                      <th className="px-4 py-3 text-[10px] font-black text-slate-500 uppercase tracking-wider w-44 border-r border-slate-200 text-right">Price (Rp)</th>
                    ...
                    {currentCategory?.items?.map((item, itemIndex) => {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                          <td className="px-4 py-2 border-r border-slate-100 text-xs font-bold text-slate-500 text-center">{itemIndex + 1}</td>
                          ...
                          <td className={`p-0 border-r border-slate-100 ${qtyInvalid ? 'bg-red-50' : ''}`}>
                            <div className="flex items-center">
                              <input
                                ...
                                disabled={!isPrivileged}
                                className={`w-16 h-full px-2 py-3 text-right text-sm font-bold tabular-nums ${qtyInvalid ? 'text-red-600' : 'text-primary'} bg-transparent focus:bg-blue-50 focus:outline-none disabled:cursor-default`}
                              />
                              <span className="text-[10px] font-bold text-slate-400 pr-2">{item.unit || 'Ls'}</span>
                            </div>
                          </td>
